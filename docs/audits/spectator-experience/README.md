...
left. The resulting spectator-host can advance completed turns and finish the
game, while the server continues to deny player-only data and actions. This is
a continuity fallback with disruption potential, not an unprompted privilege
escalation.
...
1. **Lobby and start — healthy.** The host can distinguish joined players and
   ...
   identities.
2. **Clue creation — late arrivals are players.** The landed roster architecture
   gives arrivals during clue creation a private player seat. The spectator
   boundary therefore begins once guessing is underway.
