...
Protocol version 5 combines explicit turn identity with persistent fixed-role metadata in private hinting snapshots. Deploy the frontend and game server together; older clients must reload to reconnect. No compatibility shim accepts revision-only commands.
