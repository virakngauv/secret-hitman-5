...
Room membership is independent from a Socket.IO connection. Disconnecting alone does not remove a player, so reconnecting with the same browser token restores the existing seat. On page exit, the client also sends a best-effort authenticated beacon. The server waits through a short reconnect grace period, cancels the intent when the identity resumes, ignores it while another tab for that identity remains active, and otherwise performs the normal leave transition. Reload, navigation, browser restoration, duplicate intents, and multiple tabs therefore do not rely on a guaranteed browser “closed forever” signal. If the browser cannot deliver the beacon, ordinary reconnect behavior and host controls remain the fallback.

Explicitly leaving during hinting removes the current board, clue, readiness, score, and turn-order entry without banning the identity. When the non-host departure leaves only the host in a two-player round, the server abandons the round and returns the host to a one-player lobby where Start game is disabled until another player joins. Rejoining while clue creation remains open creates a fresh participant seat and board; rejoining after guessing starts admits that identity as a spectator. A new identity joining during hinting receives a participant seat when fewer than 12 seats exist; after the cutoff, or at the participant cap, it receives spectator participation and cannot submit hints, claim cards, or enter the player standings.
...
During guessing, host removal deactivates the participant and ends their current and future guessing eligibility without rewriting accepted history. Scores, claims, and past picker attribution remain immutable. A not-yet-started authored turn is removed from the remaining rotation; an already active authored board remains intact for the other pickers or warned host advancement. The removed identity cannot act or rejoin.
