...
3. Every participant writes a hint and selects 1–5 editable words it should describe. Selected words become targets and every other non-assassin word becomes a civilian. New players may join during this phase; each receives a private 12-word board and must complete the same hint and editable-word selection before guessing can begin. A departed player who has not submitted receives an internal `PASS · 0` fallback.
